...
The plan: confirm git works → ask for the license → pick a destination that
ends up on the Mac → download with the license attached → save the license
inside the install so updates work forever after → verify → hand off to
Claude Code, where ButterCut Pro actually runs, for dependency setup.
...
- In **Cowork** (`Linux`): git is built in — this just confirms it. Move on.
...
2. One last step, in a different tab: open the **buttercut-pro** folder (it's
   ...
   **Cloud** to **Local**, and uncheck **worktree**. If the Mac asks to
   install Apple's developer tools the first time (that's git for the Mac
   itself), click **Install** and let it finish.
3. Then paste:
